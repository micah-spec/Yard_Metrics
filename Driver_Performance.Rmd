...
# Add columns back to rbind
        add_columns_back <- list(Driver = "Average", Total = NA)
...
# Filter by Employee ID        
        v8 <- v7 %>% filter(Driver %in% c(id,"Average"))
...
v10 <- v9 %>%
        ggplot(aes(x=Date,y=Count, color = Driver))+
        geom_point(size=2)+
        geom_line(size=.5)+
        scale_x_date(
                date_breaks = "2 days",
        ...
        geom_hline(aes(yintercept = 5,linetype = "5 MPH"), color = "coral") +
