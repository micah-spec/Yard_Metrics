...
```{r}
# Filter by Date
        v10 <- v9 %>% filter(between(Date, as.Date("2023-02-05"), as.Date("2023-02-12")))

# Filter by Shift
        v11 <- v10 %>% filter(Employee_ID %in% a1)
...
# Create new column with the min quantity of additional On Time moves needed to hit 5 moves/hr target required to meet 78% on time threshold 
       v14 <- v13 %>% mutate(Min_On_Time = pmax(round(.78*(Moves+Target_Moves)-On_Time,digits=0),0))
