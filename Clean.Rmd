...
#metric_vis arguments: data=dat, metric (choose one)= c("MPH","On-Time Moves")

metric_viz <- function(data=dat,metric)

# Extract individual metric from dataset
        v1 <- data[, data[2, ] == metric]
        v2 <- data[, c(2,length(data)-3)]
        v3 <- cbind(v2, v1)
...
# Change the data class from character to numeric
        c4 <- c(2:length(v4))
        v4[ ,c4] <- apply(v4[ ,c4], 2, function(x) as.numeric(as.character(x)))
...
```{r}
metric_viz <- function(data,metric){
...
# Change the data class from character to numeric
        c1 <- c(2:length(v5))
        v5[ ,c1] <- apply(v5[ ,c1], 2, function(x) as.numeric(as.character(x)))
