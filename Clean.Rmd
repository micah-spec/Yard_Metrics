...
# Extract completed moves metric from dataset
        ...
        colnames(cms4)[2] <- "Completed Moves"
...
# Extract Hours Worked metric from dataset
        ...
        hrw5 <- hrw4[,-2] %>%
                pivot_longer(
                        cols = !"Employee_ID",
                        names_to = "Date",
                        values_to = "Hours Worked",
                )
        ## Convert Date column to date data-class
        hrw5$Date <- mdy(hrw5$Date)
        hrw5 <- as.data.frame(hrw5)

# Extract MPH metric from dataset
        mph1 <- dat[, dat[2, ] == "MPH"]
 ...
 # Extract OTP metric from dataset
...
v10 <- v9 %>%
        ...
        geom_hline(yintercept = 5, color = "green") +
        scale_color_manual(values = c("red", "blue")) +
        labs(y="MPH",title = "Moves/Hr with Shift Average")+
        theme_light()
v10     
}
```
...
labs(x="Date",y="Moves",title = "Total Moves")
  theme_light()
v14
...
```{r}
shift_average <- c(sum(v8$Total/length(shift)))
```
       
# Create Summary
        shift_average <- c(sum(v8$Total/length(shift)),mean(v8$MPH),sum(v8$Hours)/length(shift))
